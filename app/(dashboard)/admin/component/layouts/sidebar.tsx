...
const Sidebar = () => {
    ...
    <Image src="../images/dashboard/logo-admin.svg" alt="logo admin" width={214} height={36}/>
    </div>
        ...
        {menuItems.map((item, index) => {
            ...
            return (
                <Link 
                    href= {item.link}
                    key= {index}
                    className= {`flex gap-2 items-center py-3 px-4.5 rounded-lg font-medium duration-300 ${isActive ? "bg-primary/15 text-primary" : "hover:bg-gray-100"}`}>
                    <item.icon size={24}/>
                    <span>{item.name}</span>
                    </Link>
